...
try:
    # requests uses simplejson when it is installed, and then raises *its*
    # JSONDecodeError - which does not inherit from the stdlib one.
...
class Discourse:
    def __init__(
        self, base_url, username, api_key,
        raise_for_rate_limit=True, timeout=10,
    ...
    def _request(self, method, url, data=None, params=None, attempt=0):
        ...
        r = requests.request(
            method, url, json=data, params=params, headers=self._headers, timeout=self._timeout
        )
    ...
    def _wait_for_rate_limit(self, response, method, url, data, params):
        ...
        try:
            ...
            # Not every rate limit response carries extras.wait_seconds
            wait_seconds = 10
